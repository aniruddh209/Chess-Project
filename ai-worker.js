/**
 * AI Worker Thread — runs minimax computation off the main event loop.
 * Receives: { fen, difficulty }
 * Returns:  { move } or { move: null }
 */
...
function getAIMove(chessInstance, difficulty) {
  ...
  // Among moves with similar evaluation, pick randomly for variety
  const threshold = 15;
...
// ============================================================
//  MESSAGE HANDLER — receives { fen, difficulty }, returns { move }
// ============================================================

parentPort.on("message", (msg) => {
  try {
    const chess = new Chess(msg.fen);
    const aiMove = getAIMove(chess, msg.difficulty);
    if (aiMove) {
      parentPort.postMessage({
        move: { from: aiMove.from, to: aiMove.to, promotion: aiMove.promotion || "q" },
      });
    } else {
      parentPort.postMessage({ move: null });
    }
  } catch (err) {
